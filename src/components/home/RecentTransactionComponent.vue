<template>
    ...
    <section class="mb-16">
        ...
        <transition active-class="animate__animated animate__fadeInUp animate__slow" mode="out-in">
            <div v-if="!isLoading">
                <div v-for="(item, index) in transactions" :key="index" class="flex items-center mb-3 justify-between p-4 bg-gray-50 rounded-2xl">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h4 class="font-semibold text-base md:text-lg">{{ item.notes }}</h4>
                            <p class="text-xs sm:text-sm text-gray-500">{{ item.created_at }}</p>
                        </div>
                    </div>
                    <span class="font-semibold text-gray-900 text-lg">- IDR {{ $helper.currency(item.amount) }}</span>
                </div>
            </div>
